...
export const POST = async (req: Request) => {
  ...
  const reponse = await openai.chat.completions.create({
    ...
    messages: [querySystemMessage, messages.at(-1)],
    response_format: { type: "json_object" },
    tools: [...functions],
    tool_choice: {
      type: "function",
      function: { name: "generate_user_queries" },
    },
  });
