...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    if (!error && user) {
      ...
      const [first, last] = user.user_metadata.full_name
        .replace(" ", "@")
        .split("@");
      ...
      if (
        !res.success &&
        res.message !== "Email or userId is already on list."
      ) {
        console.log(res);
        console.error(
          `Couldn't subscribe user ${user.id} with email ${user.email} to Loops.`,
        );
      }
