...
export const POST = async (req: Request, res: Response) => {
  ...
  if (!userData[0]) {
    return new Response(JSON.stringify("User has not linked accounts"), {
      status: 500,
    });
  }
  ...
  const { data: games, error: gamesError } = await supabase
    ...
    .select("*")
    .or(
      `white.in.(${chesscom_name}), black.in.(${chesscom_name}), white.in.(${lichess_name}), black.in.(${lichess_name})`,
    )
  ...
  const { data: analysis, error: analysisError } = await supabase
    .from("user_analysis")
    .upsert([{ uuid: data.user.id, playstyle, weaknesses }])
    .select();
