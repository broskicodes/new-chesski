...
export const StockfishProvider = ({ children }: PropsWithChildren) => {
  ...
  const onMessage = useCallback(
    (event: MessageEvent<string>) => {
      ...
      if (event.data === "uciok") {
        ...
        if (
          (!moveTime && event.data.includes(` depth ${MAX_DEPTH}`)) ||
          (moveTime &&
            parseInt(event.data.split(" time ")[1].split(" ")[0]) >= moveTime)
        ) {
          const pv = event.data.split(" pv ")[1];
          // const bestMove = pv.split(" ")[0];
          const multiPv = parseInt(
            event.data.split(" multipv ")[1].split(" ")[0],
          );
  ...
  const initEngine = useCallback(
    (limitStrength: boolean, skillLvl?: SkillLevel, moveTime?: number) => {
      if (!worker) {
  ...
  const startSearch = useCallback(() => {
    ...
    worker.postMessage(
      `go ${moveTime ? `movetime ${moveTime}` : `depth ${MAX_DEPTH}`}`,
    );
